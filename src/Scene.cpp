...
bool Scene::initialiserFenetre() {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    std::cerr << "Error during the initialisation of SDL: " << SDL_GetError() << std::endl;
    SDL_Quit();
    return false;
  }
  ...
  if (m_fenetre == 0) {
    std::cerr << "Error during the creation of the window: " << SDL_GetError() << std::endl;
    SDL_Quit();
    return false;
  }

  // Creation of the OpenGL contexte
...
bool Scene::initGL() {
  ...
  if(initialisationGLEW != GLEW_OK) {
    std::cerr << "Error in the initialisation of GLEW : " << glewGetErrorString(initialisationGLEW) << std::endl;
    SDL_GL_DeleteContext(m_contexteOpenGL);
    SDL_DestroyWindow(m_fenetre);
    SDL_Quit();
    return false;
...
void Scene::animate() {
  ...
  if (running || step_by_step > 0) {
    if ((int)time_ >= -mpm_conf::replay_speed_) {
      time_ += mpm_conf::replay_speed_;
    } else {
      time_ = 0;
    }
    if (toggle_record) {
...
void Scene::bouclePrincipale() {
  ...
  while(!end_) {
    ...
    if (back) {
      sim->backward(back+fabs(mpm_conf::replay_speed_));
      back = 0;
    }
...
void Scene::printCameraPos() {
  ...
  INFO(0, "<camera>\n <position> "<<m_camera.getPosition()[0]<<" "<<m_camera.getPosition()[1]<<" "<<m_camera.getPosition()[2]<<
       ...
       "\n</camera>");
  INFO(0, "Mistuba:\n"<<"<lookat target=\""<<10*target[0]<<", "<<10*target[1]<<", "<<10*target[2]<<"\" origin=\""<<10*pos[0]<<", "<<10*pos[1]<<", "<<10*pos[2]<<"\" up=\""<<up[0]<<", "<<up[1]<<", "<<up[2] <<"\"/>");
}
