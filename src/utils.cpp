...
VEC2 utils::projectionOrtho(const VEC2 p, const VEC2 origine, const VEC2 dir, FLOAT & coef) {
  ...
  IS_DEF(dir(1));
  TEST(dir.norm() > 0);
  //INFO("projectionOrtho (p o dir)", p<<"\n\n"<<origine<<"\n\n"<<dir);
  MAT3 M(3, 3);
  assert(dir.norm() != 0);
  M << 1, 0, -dir(0), 
    0, 1, -dir(1), 
...
FLOAT utils::aire(const VEC3 x1, 
	    ...
	    const VEC3 x3) {  
  // FLOAT sign;
  // if ((x3 - x1).dot(x2 - x1) < 0) {
  //   sign = 1;
  // } else {
  //   sign = -1;
  // }
  return fabs(((x3 - x1).cross(x2 - x1)).norm()/2.0);
  // return ((x2(0) - x1(0))*(x3(1) - x1(1)) - 
  // 	  (x3(0) - x1(0))*(x2(1) - x1(1)))/2;
}
...
FLOAT utils::dihedral_angle(VEC3 e1, 
  ...
  //  INFO("n1 n2", n1.dot(n2)<<" "<<acos(n1.dot(n2))<<"\n\n"<<n1<<"\n\n"<<n2);
  // IS_DEF(angle);
  TEST(e1 != e2);
  TEST(e1 != s2);
  TEST(e1 != s1);
  ...
  //  ERREUR(angle < 3.0 * M_PI/4.0, "angle to big", "utils::dihedral_angle", "n1 n2 : "<< n1.dot(n2)<<" "<<acos(n1.dot(n2))<<"\n\n"<<n1<<"\n\n"<<n2);
  return angle;
} 
...
// void utils::rotate(VEC3 axe, FLOAT angle, VEC3 & v) {
//   AngleAxis<FLOAT> aa = AngleAxis<FLOAT>(angle, axe);
...
// bool utils::isOnTheLeft(const VEC2 & pt, const VEC2 & seg_b, const VEC2 & seg_e) {
//   VEC2 dir = seg_e - seg_b;
//   VEC2 n = VEC2(-dir(1), dir(0));

//   VEC2 dir2 = pt - seg_b;
  
//   bool left = dir2.dot(n) > 0;

//   if (left) {
//     TEST(utils::aire(seg_b, seg_e, pt) > -EPS);
//   } else {
//     TEST(utils::aire(seg_b, seg_e, pt) < EPS);
//   }
//   return left;
...
bool utils::computeEigen(MAT2 & M, VEC2 & eigen_val, MAT2 & eigen_vec) {
  ...
  // INFO("det", det<<" "<<pow(trace, 2)-4.0*det);
  // INFO("M", M);
  // if (det < 0) {
  //   det = 0;
  // }
  ...
  if (success) {
    ...
    //INFO("eigen_val",  eigen_val(0)<<" "<< eigen_val(1));
  if (M(1, 0) != 0) {
