...
Particule::Particule(FLOAT mass, FLOAT vol, VEC3 p, VEC3 velo, Shader* shader) :
  Object(shader), m(mass), v(vol), v0(vol), pos(p), vel(velo), F_p(MAT3::Identity(3, 3)), F_e(MAT3::Identity(3, 3)) {
...
MAT3 Particule::energyDerivative(VEC3 sigma) {
  ...
  return 2*mpm_conf::mu_*alpha*invSigma*lnSigma + mpm_conf::lambda_*alpha*tr*invSigma;
...
void project(VEC3 sigma, FLOAT alpha, VEC3 & T, FLOAT & plastic_def) {
  ...
  //   }
  // }
  double smax = 1 + 7.5e-3;
  double smin = 1 - 2.5e-2;
   for (uint i = 0; i < 3; ++i) {
     T(i) = sigma(i);
     if (sigma(i) < smin) {
       T(i) = smin;
